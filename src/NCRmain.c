...
int main (int argc, char *argv [])
{
	...
	for (argPos = 1;argPos < argNum; )
	{
		...
		{
			CMmsgPrint (CMmsgInfo,"%s [options] <ncgis grid> <ascii table>\n", CMprgName(argv [0]));
			CMmsgPrint (CMmsgInfo,"     -n,--network\n");
			CMmsgPrint (CMmsgInfo,"     -h,--help\n");
			return (0);
		}
		if ((argv [argPos][0] == '-') && (strlen (argv [argPos]) > 1))
		{ fprintf (stderr,"Unknown option: %s!\n",argv [argPos]); return (-1); }
		argPos++;
	}
	if (argNum != 3)
	{ CMmsgPrint (CMmsgInfo,"%s: %s!\n", CMprgName (argv [0]), argNum < 3 ? "Missing runoff" : "Extra arguments"); return (-1); }
	...
	{ NetworkCellFree (firstCell); return (-1); }
	if ((timeStepNum = inputTimeStepNum (runoff)) == 0)
	{ NetworkCellFree (firstCell); inputClose (runoff); return (-1); }
	...
	for (timeStep = 0;timeStep < timeStepNum; ++timeStep)
	{
		...
		outputWrite (outflow, timeStep, firstCell);
	}
