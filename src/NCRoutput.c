...
void *NCRoutputOpen(NCRnetwork_t *network, const char *target, const char *varName) {
    ...
    for (cellId = 0; cellId < network->CellNum; cellId++) {
        ...
        if (cell->ToCellId > 0) {
            ...
            if ((fabs(cell->Lon - toCell->Lon) > 0.0) && (fabs(cell->Lon - toCell->Lon) < dx))
                dx = fabs(cell->Lon - toCell->Lon);
            if ((fabs(cell->Lat - toCell->Lat) > 0.0) && (fabs(cell->Lat - toCell->Lat) < dy))
                dy = fabs(cell->Lat - toCell->Lat);
        }
    }
...
bool NCRoutputWrite(void *ptr, size_t time_step, NCRnetwork_t *network) {
    ...
    count[0] = 1;
    start[1] = (size_t) 0;
    count[1] = output->RowNum;
    start[2] = (size_t) 0;
    count[2] = output->ColNum;
