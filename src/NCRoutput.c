...
void *outputOpen (NetworkCell_t *firstCell, const char *target, const char *varName)
{
	...
	{ CMmsgPrint (CMmsgSysError,"Memory allocation error in: %s:%d\n",__FILE__,__LINE__); return ((void *) NULL); }

	if ((status = nc_create (target, NC_CLOBBER, &ncid))   != NC_NOERR)
	{ CMmsgPrint (CMmsgAppError, "NCError \"%s\" in: %s:%d\n", nc_strerror(status),__FILE__,__LINE__); free (array); return ((void *)  NULL); }
	if ((status = nc_def_dim (ncid,"time", NC_UNLIMITED,  dimids + 0)) != NC_NOERR)
	{ CMmsgPrint (CMmsgAppError, "NCError \"%s\" in: %s:%d\n", nc_strerror(status),__FILE__,__LINE__); nc_close (ncid); free (array); return ((void *) NULL); }
	...
	{ CMmsgPrint (CMmsgSysError,"Memory allocation error in: %s:%d\n",__FILE__,__LINE__); nc_close (ncid); free (array); return ((void *) NULL); }
